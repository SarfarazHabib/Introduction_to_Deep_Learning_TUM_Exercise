...
def cross_entropoy_loss_vectorized(W, X, y, reg):
    ...
    #Calculating scores
    score = np.dot(X,W)
    #Calculating exponential of scores; to avoid numerical problem with exp,
    #normalizing the score with max as zero
    exp_scores_stable = np.exp(score - np.max(score, axis=1, keepdims=True))

    #computing the probabilities
    softmax_probab = exp_scores_stable/ np.sum(exp_scores_stable, axis=1, keepdims=1)

    #computing loss
    loss = np.sum(-np.log(softmax_probab[range(train_num), y])) / train_num
    loss += 0.5 * reg * np.sum(W ** 2)

    d_scores = softmax_probab
    d_scores[range(train_num), y] -= 1
    dW = np.dot(np.transpose(X), softmax_probab) / train_num
    ############################################################################
    #                          END OF YOUR CODE                                #
    ############################################################################
